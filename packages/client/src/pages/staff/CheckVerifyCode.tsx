...
export const CheckVerifyCode = ({ client, trpc }) => {
    const [code, setCode] = useState("");
    const [mobile, setMobile] = useState("");
    const [result, setResult] = useState("");

    const getOtpStatus = trpc.useMutation(["staff.checkVerifyCode"]);
        ...
        <>
            <div className="h-screen flex">
                <div className="m-auto justify-center items-center text-gray-800 px-80 flex-col space-y-4">
                    ...
                    <Input type={"text"} val={code} fn={(e) => setCode(e.target.value)} placeholder={"OTP Code"} style={"p-5 border-2 border-gray-300 rounded-lg w-full"} />
                    <Button type={"button"} val={"Check OTP"} onClickAction={checkOTP} style={"inline-block w-full px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"} />
                    <Link to="/generate"><div className="text-purple-600 hover:text-purple-700 transition duration-300 ease-in-out mb-4 mt-2">Go Generate {">"}</div></Link>
                </div>
            </div>
